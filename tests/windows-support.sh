...
cat > "$PLUGIN_ROOT/runtime/clover-hook-windows-amd64.exe" <<'EOF'
#!/usr/bin/env bash
printf 'fake-hook:%s\n' "$*"
EOF
cat > "$PLUGIN_ROOT/runtime/clover-hook-windows-arm64.exe" <<'EOF'
#!/usr/bin/env bash
printf 'fake-hook:%s\n' "$*"
EOF
cat > "$PLUGIN_ROOT/runtime/clover-hook-darwin-arm64" <<'EOF'
#!/usr/bin/env bash
printf 'fake-hook:%s\n' "$*"
EOF
chmod +x "$PLUGIN_ROOT"/runtime/*
